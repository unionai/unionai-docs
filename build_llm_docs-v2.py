...
class LLMDocBuilder:
    ...
    def find_variants(self) -> List[str]:
        ...
        version = self.detect_version()
        dist_path = self.base_path / "dist" / "docs" / version
...
"""
    ...
    def create_discovery_files(self, base_path: Path, variants: List[str], version: str) -> None:
        """Create hierarchical discovery files for LLM documentation."""

        # Root level discovery file (/docs/llms.txt)
        root_content = self.create_root_discovery_content(variants, version)
        ...
        # Version level discovery file (/docs/v1/llms.txt or /docs/v2/llms.txt)
        version_content = self.create_version_discovery_content(variants, version)
        version_file = base_path / 'dist' / 'docs' / version / 'llms.txt'

        with open(version_file, 'w', encoding='utf-8') as f:
            f.write(version_content)
        print(f"✅ Created {version} discovery: {version_file}")

    def create_root_discovery_content(self, variants: List[str], current_version: str) -> str:
...
def main():
    ...
    # Step 2: Detect version and find variants
    version = builder.detect_version()
    variants = builder.find_variants()
    if not variants:
        print("❌ No variants found")
        return 1

    print(f"📋 Detected {version} structure")
    print(f"📋 Found variants: {variants}")

    # Step 3: Build consolidated documents
    for variant in variants:
        consolidated_content = builder.build_consolidated_doc(variant, version)

        if consolidated_content.strip():
            # Create output file
            output_file = base_path / 'dist' / 'docs' / version / variant / 'llms-full.txt'
            ...
            redirect_file = base_path / 'dist' / 'docs' / version / variant / 'llms.txt'
    ...
    builder.create_discovery_files(base_path, variants, version)
