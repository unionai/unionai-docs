...
def process_page_node(page_node: dict, current_variant: str, parent_path: str, parent_variants: list, run_commands: dict) -> str:
    ...
    with ThreadPoolExecutor() as executor:
        future_to_child = {executor.submit(process_page_node, child_page_node, current_variant, path, variants, run_commands): child_page_node for child_page_node in children}
        for future in as_completed(future_to_child):
            toc_entry = future.result()
            if toc_entry:
                toctree += toc_entry + '\n'
...
def process_variant(variant: str, page_node: dict, run_commands: dict):
...
def process_project():
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_variant, variant, page_node, run_commands) for variant in ALL_VARIANTS]
