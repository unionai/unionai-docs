...
A resolved `Volume` plus the IO to inspect and walk its lineage.

Construct via `from_action` (resolve off a run/action), `from_volume_value`
(wrap an in-hand Volume), or `from_volume_json` (a serialized value on disk).
...
| [`from_volume_json()`](#from_volume_json) | Load a serialized Volume value (``.json``) from disk. |
| [`from_volume_value()`](#from_volume_value) |  |
| [`lineage()`](#lineage) | Full version chain, newest first, walked via ``parent_produced_by.locator``. |
| [`lineage_has_navigable_parent()`](#lineage_has_navigable_parent) | True iff the immediate parent in ``lineage`` can actually be opened — it loaded (no ``error``), has its own index, and is a *distinct* version. |
| [`parent()`](#parent) | The immediate parent version as a new `VolumeExplore`, or ``None``. |
...
```python
def lineage(
    current_reader: Optional[Any] = None,
) -> List[dict]
```
...
def parent()
```
The immediate parent version as a new `VolumeExplore`, or ``None``.
