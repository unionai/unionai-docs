...
When using the `deployed-task` subcommand, `--run-project` and `--run-domain` specify the [project-domain pair](../user-guide/getting-started/projects-and-domains) in which to *execute* the task. This lets you run a deployed task in a different project or domain than the one configured in your `config.yaml`:
