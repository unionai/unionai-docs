...
For example, create a new role to view and register flyte inventory:
    ...
    bin/flytectl create role --name "Registration Role" --actions view_flyte_inventory,view_flyte_executions,register_flyte_inventory
...
```shell
$ flytectl/bin create role --roleFile role.yaml
```
...
```shell
$ flytectl/bin create role --genFile --roleFile role.yaml
```
...
```shell
$ flytectl/bin create policy --help
```
