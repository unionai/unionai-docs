...
| [`AgentTool`](../flyte.ai.agents/agenttool) | A normalized tool descriptor used by `Agent`. |
| [`LLMMessage`](../flyte.ai.agents/llmmessage) | Provider-agnostic shape returned by `LLMCallable`. |
...
| [`ToolFn`](../flyte.ai.agents/toolfn) | The tool under invocation, handed to a `ToolCallHandler`. |
...
| [`MemoryStoreError`](../flyte.ai.agents/memorystoreerror) | Base class for `MemoryStore` errors. |

### Methods

| Method | Description |
|-|-|
| [`tool()`](#tool) | Wrap a task, ``@flyte.trace`` helper, plain callable, or ``LazyEntity`` as an `AgentTool`. |
...
The wrapped task is still registered with its `TaskEnvironment`
...
| `call_handler` | `ToolCallHandler \| None` | Optional async interceptor ``(call_llm, tool_fn, **kwargs)`` that customizes how the tool is invoked. See `ToolCallHandler` and `ToolFn`. |

**Returns:** An `AgentTool` (direct call) or a decorator returning one.
