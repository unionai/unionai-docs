...
| `tasks` | `Optional[List[Any]]` | List of external functions (tasks, durable ops) available inside the sandbox. Each item's `__name__` is used as the key. |
...
| `output` | `type` | The return type (default `NoneType`). |
| `tasks` | `Optional[List[Any]]` | List of external functions (tasks, durable ops) available inside the sandbox. Each item's `__name__` is used as the key. |
| `name` | `str` | Task name (default `"sandboxed-code"`). |
| `timeout_ms` | `int` | Sandbox execution timeout in milliseconds. |
