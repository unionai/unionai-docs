...
| `stats` | `taggable.TaggableStats` | A handle to a special statsd object that provides usefully tagged stats. TODO: Usage examples and better comments |
| `task_id` | `typing.Optional[_identifier.Identifier]` | At production run-time, this will be generated by reading environment variables that are set by the backend. |
| `timeline_deck` | `'TimeLineDeck'` |  |
| `working_directory` | `str` | A handle to a special working directory for easily producing temporary files. TODO: Usage examples |
