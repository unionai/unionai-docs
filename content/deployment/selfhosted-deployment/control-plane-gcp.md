...
For production deployments, use cert-manager with a self-signed `ClusterIssuer` or your organization's CA. See the `extraObjects` section in `values.gcp.selfhosted-intracluster.yaml` for an example configuration.
...
Create a customer overrides file `values.gcp.selfhosted-customer.yaml`:
...
3. `values.gcp.selfhosted-customer.yaml` — Your environment-specific overrides
