...
As described in the [Architecture Overview](https://docs.flyte.org/en/latest/concepts/architecture.html), the Flyte control plane (`flyteadmin`) sends workflows off to the Compute Plane (`flytepropeller`) for execution.
The compute plane fulfills these workflows by launching pods in Kubernetes.
...
To address this, you can deploy Flyte's compute plane to multiple Kubernetes clusters.
The control plane (`flyteadmin`) can be configured to submit workflows to these individual compute planes.
...
2. Two IAM Roles configured: one for the control plane components, and another for the compute plane where the worker Pods and `flytepropeller` run.
...
3. Open the `values-dataplane.yaml` file and add the following contents:
   ...
   This step is needed so the `flytepropeller` instance in the compute plane cluster is able to send notifications back to the `flyteadmin` service in the control plane.
   ...
   The `catalog` service in the control plane cluster can, for instance, be made available to the `flytepropeller` services in the compute plane clusters with an internal load balancer service.
...
4. Install the Flyte compute plane Helm chart. Use the same base `values` file you used to deploy the control plane:
...
For `flyteadmin` to access and create Kubernetes resources in one or more Flyte compute plane clusters, it needs credentials to each cluster.
Flyte makes use of Kubernetes Service Accounts to enable every control plane cluster to perform authenticated requests to the Kubernetes API Server in the compute plane cluster.
The default behavior is that the Helm chart creates a [ServiceAccount](https://github.com/flyteorg/flyte/blob/master/charts/flyte-core/templates/admin/rbac.yaml#L4)in each compute plane cluster.
...
1. Use the following manifest to create a long-lived bearer token for the `flyteadmin` Service Account in your compute plane cluster:
...
3. Copy the bearer token of the first compute plane cluster's secret to your clipboard using the following command:
...
4. Go to `secrets.yaml` and add a new entry under `stringData` with the compute plane cluster token:
...
9. Add the compute plane IAM Role as the `defaultIamRole` in your Helm values file. [See AWS example](https://github.com/flyteorg/flyte/blob/97a79c030555eaefa3e27383d9b933ba1fdc1140/charts/flyte-core/values-eks.yaml#L351-L365)
...
2. Install the compute plane Helm chart following the steps in the [Compute plane deployment](#compute-plane-deployment) section.
