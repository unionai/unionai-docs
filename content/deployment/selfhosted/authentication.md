...
The identity service then serves the `EAGER_API_KEY` from this secret instead of contacting the IdP.
...
| `eager-client-creds` | `<controlplane-namespace>` | `client_id`, `client_secret` | EAGER app (App 5) credentials, referenced by `services.identity.apiKeyOverrides.EAGER_API_KEY`. The data-plane task-pod key is then minted automatically by the operator (`config.operator.apiKey.enabled`). |
