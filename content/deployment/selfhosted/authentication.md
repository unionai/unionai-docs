...
Note the **Client ID** (set as `flyte.configmap.adminServer.auth.userAuth.openId.clientId`) and the **Client Secret** (stored in the dedicated `flyteadmin-oidc-client-secret` Secret under the key `client_secret`, read via `userAuth.openId.clientSecretFile` — see [Step 3](#step-3-create-kubernetes-secrets-control-plane)).
...
Point flyteadmin at the browser-login secret by mounting it and setting `clientSecretFile` in your control plane values (see the chart's `examples/values.oidc-client-secret.yaml`):
...
> [!NOTE]
> flyteadmin's `flyte-admin-secrets` (token-signing and cookie keys) is created automatically by an init container — you don't create it, and the browser-login client secret does **not** go there. Keeping the OIDC client secret in its own dedicated secret avoids colliding with those auto-generated keys.
...
Eager (v2 / actions) execution requires task pods to authenticate to the control plane using the EAGER app (App 5) credentials, surfaced as the `EAGER_API_KEY`. On a self-hosted deployment you provide those credentials once, and the platform provisions and injects the key automatically — no manual encoding or per-pod secret.

### Control plane: seed the EAGER client credentials

The identity service normally mints system keys by registering an OAuth client on the identity provider. Self-hosted deployments run against an operator-controlled IdP (Okta, Entra ID) where the control plane cannot register clients, so instead you seed the EAGER app (App 5) credentials as a Kubernetes secret and point the identity service at it with `apiKeyOverrides`:

```shell
# Org-wide default credentials — serve any data plane cluster without its own entry
kubectl create secret generic eager-client-creds \
  --from-literal=client_id='<EAGER_CLIENT_ID>' \
  --from-literal=client_secret='<EAGER_CLIENT_SECRET>' \
  -n <controlplane-namespace>

# Optional: distinct credentials scoped to a single data plane cluster (dp-1)
kubectl create secret generic eager-dp1-creds \
  --from-literal=client_id='<DP1_EAGER_CLIENT_ID>' \
  --from-literal=client_secret='<DP1_EAGER_CLIENT_SECRET>' \
  -n <controlplane-namespace>
```

`apiKeyOverrides` is a **list** of entries, each identified by `key` plus an optional `clusterName`:

```yaml
services:
  identity:
    apiKeyOverrides:
      # Org-wide default: an entry with no clusterName serves every data plane
      # cluster that doesn't have its own entry below.
      - key: EAGER_API_KEY
        existingSecret:
          name: eager-client-creds         # the secret created above (required)
      ...
      # Cluster-specific override: takes precedence for data plane cluster "dp-1".
      # Add one entry per cluster that needs distinct credentials; omit this block
      # entirely if a single org-wide key is enough.
      - key: EAGER_API_KEY
        clusterName: dp-1
        existingSecret:
...
The identity service then serves the `EAGER_API_KEY` from these secrets instead of contacting the IdP. For a given data plane it prefers the entry whose `clusterName` matches that cluster and falls back to the cluster-nameless (org-wide) entry. Each entry mounts at its own path — `/etc/secrets/apikey/EAGER_API_KEY` for the org-wide default and `/etc/secrets/apikey/EAGER_API_KEY-dp-1` for the `dp-1` override.

> [!NOTE]
> `apiKeyOverrides` is reference-only: it names a pre-created secret and never holds a client secret inline, because this values file renders into a committed overlay. For production, sync `eager-client-creds` from your cloud secret manager (AWS Secrets Manager, GCP Secret Manager, Azure Key Vault) with External Secrets Operator.

### Data plane: enable the operator eager-key bootstrap

With the credentials available on the control plane, the operator mints the eager API key and writes it to the task-pod secret store, where the pod webhook injects it into task pods:
...
> [!NOTE]
> `config.operator.apiKey.enabled` is the chart default as of the v2 / actions switch-over, so you may not need to set it explicitly. It requires the embedded secret manager (`proxy.secretManager.enabled`, also a chart default). With this approach no base64 encoding or manual secret creation is needed on the data plane.

<details>
<summary>Alternative: manual EAGER_API_KEY secret (legacy)</summary>

If you are not using the operator eager-key bootstrap, create the task-pod secret by hand. The `EAGER_API_KEY` is a base64-encoded string containing the EAGER app credentials:
...
The exact secret name and key depend on your deployment's embedded K8s secret manager configuration (the name is typically an MD5 hash of a logical identifier). Contact {{< key product_name >}} support for the exact values for your organization.
...
|-------------|-----------|------|--------|
| `flyteadmin-oidc-client-secret` | `<controlplane-namespace>` | `client_secret` | Browser login app (App 1) secret |
| `flyte-admin-secrets` | `<controlplane-namespace>` | token-signing / cookie keys | Auto-generated by the flyteadmin init container (do not create) |
...
| `eager-client-creds` | `<controlplane-namespace>` | `client_id`, `client_secret` | EAGER app (App 5) credentials, referenced by an `EAGER_API_KEY` entry in the `services.identity.apiKeyOverrides` list (one org-wide default, plus optional per-cluster entries). The data-plane task-pod key is then minted automatically by the operator (`config.operator.apiKey.enabled`). |
...
| EAGER_API_KEY | Seed the EAGER client secret + reference via `apiKeyOverrides`; the operator bootstraps the task-pod key | Automatic — `uctl create apikey` generates and provisions |
