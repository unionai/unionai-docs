...
- **Intra-cluster (special case)** — both planes in the same Kubernetes cluster, communicating over cluster-local Services. The simplest topology; used by the [Getting started](./getting-started) walkthrough and for footprint-constrained or evaluation deployments. Layer `examples/values.{aws,gcp}.intracluster.yaml` on top of the canonical `values.{aws,gcp}.yaml` to enable it. See [Infrastructure requirements → Intra-cluster topology](./infrastructure-requirements#intra-cluster-topology) for the differences from separate-cluster.
