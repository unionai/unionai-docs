...
Regardless of cloud provider, Union.ai enforces the same security model: control plane / data plane separation, no customer data ever transiting the control plane (under Zero Trust), presigned URLs for bulk data access direct to the customer's object store, structured task I/O and logs served directly from the data plane through the [Direct-to-DataPlane tunnel](../architecture/network) (or, under the [Sovereign Data Plane](../architecture/sovereign-data-plane) tier, through a customer-managed internal load balancer), outbound-initiated CP/DP connectivity from the data plane, RBAC-based access control, and encryption at rest and in transit. Cloud-specific implementations (IAM roles, encryption services, log aggregators, and secrets managers) are abstracted by the platform while maintaining native integration with each provider's security services. A workflow running on AWS receives the same separation guarantees as one running on GCP or Azure; only the underlying cloud primitives differ. Data residency at rest is maintained within the customer's chosen cloud and region.
