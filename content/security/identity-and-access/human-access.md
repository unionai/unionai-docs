...
Self-managed: Union.ai has no IAM roles, no VPN, no SSH keys, and no kubectl access to the customer's cluster. Every channel between Union and the customer is initiated FROM the customer's data plane. Union.ai cannot initiate connections TO the customer's infrastructure under any tier.
