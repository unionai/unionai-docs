...
> [!WARNING]
> **Audit finding (ref #7, #11):** "Only the metadata required for orchestration" understates what the control plane database stores. The database stores as explicit columns: task/workflow/execution/launch plan identifiers (including **task function names**, workflow names, and execution names), execution state (phase, timestamps, cluster, error kind/code), user identity (who launched each execution), and task type strings and descriptions. Task definition closures are stored as binary blobs containing not just container image references and typed interfaces, but also: environment variables (`Container.env`), default input literal values (`default_inputs`), SQL query statements (`Sql.statement`), full Kubernetes pod specs (`K8sPod.pod_spec`), arbitrary plugin data (`TaskTemplate.custom`), and arbitrary config key-value pairs (`TaskTemplate.config`). Launch plan specs contain default literal values. Error messages from task executions (up to 4KB) and Kubernetes event messages are persisted via the events system. These fields can contain sensitive customer data. All data is encrypted at rest (AWS RDS AES-256/KMS).
