...
> [!WARNING]
> **Audit finding (ref #7, #11):** "The control plane never stores customer data payloads" needs qualification. The database stores as explicit columns: task/workflow/execution/launch plan identifiers (including task function names, workflow names, execution names), user identity, execution state, and descriptions. Task definition closures stored as blobs contain environment variables, default input literal values, SQL query statements, K8s pod specs, arbitrary plugin configuration, and config key-value pairs -- fields that can contain sensitive customer data. Launch plan specs also contain default literal values. Error messages from task executions (up to 4KB) and Kubernetes event messages are persisted via the events system. All data is encrypted at rest (AWS RDS AES-256/KMS).
