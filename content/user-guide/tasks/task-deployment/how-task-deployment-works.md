...
{{< variant union >}}
{{< markdown >}}
When `image.builder` is set to `local` in [your `config.yaml`](../../get-started/run-modes/running-remote), images are built on your local machine using Docker. This approach:
{{< /markdown >}}
{{< /variant >}}
...
{{< variant union >}}
{{< markdown >}}
When `image.builder` is set to `remote` in [your `config.yaml`](../../get-started/run-modes/running-remote), images are built on cloud infrastructure. This approach:
{{< /markdown >}}
{{< /variant >}}
