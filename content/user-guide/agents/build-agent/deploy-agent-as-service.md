...
> [!NOTE]
> When the webhook app submits runs on behalf of incoming requests, it needs valid {{< key product_name >}} credentials. Use passthrough auth (a `FastAPIPassthroughAuthMiddleware` and `flyte.init_passthrough`) so the run is submitted with the caller's identity. See [FastAPI apps](../../apps/native-app-integrations/fastapi-app).
...
> [!TIP]
> See [Build Apps](../../apps/build-apps/_index) and [Configure Apps](../../apps/configure-apps/_index) for more details on hosting services on {{< key product_name >}}.
