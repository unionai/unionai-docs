...
This page takes you from no account to a run you can see in the Union console, in a few minutes. Nothing runs on a cluster yet. Your first workflow executes on your own machine and reports its progress to Union, so you can see what Union does before you connect any infrastructure.
...
Go to [signup.union.ai](https://signup.union.ai) and select **Continue with Google**. Choose the account you want to use for Union.

<!-- screenshot: sign-in page, frame on "Continue with Google". Capture on staging; crop the URL bar. -->

## Create your organization

An organization is your top-level workspace in Union. It is where your projects, workflows, resources, and team members live.

1. **Organization name.** This becomes your organization's web address, so it must be unique across Union, and it cannot be changed later. Use lowercase letters, digits, and hyphens. As you type, Union checks whether the name is available.
...
2. **Preferred Union region.** This is where your control plane runs. The control plane is the Union service that manages your workflows, metadata, and user interface. If you later connect a cluster of your own, choose the region closest to it. If you are not sure, keep the default.

3. Select **Create Organization**.

Union sets up your organization in about thirty seconds. You'll see each step complete: receiving the request, creating the organization, setting up sign-in, preparing your workspace, and finalizing.

<!-- screenshot: provisioning phases mid-way, frame on the phase list. -->

## Sign in to your organization

When setup finishes, Union takes you to your new organization's sign-in page. Select **Continue with Google** and choose the same account again.
...
You land in the Union console. Your organization's address is shown at the top of the page, in the form `my-org.hosted.unionai.cloud`. You'll need it in the next step.

> [!NOTE] You don't need a cluster yet
> The console first asks you to set up a cluster pool. That is where you connect your own Kubernetes cluster, and you can come back to it any time. For your first run, skip it: select **Projects** in the sidebar. Your organization already has a `default` project ready to use.
...
The example fans a small computation out over a list of inputs with `flyte.map` and averages the results. The path Union prints is the run's page in the console.
...
Everything you see here came from a run on your own machine. Union recorded it as it happened.
