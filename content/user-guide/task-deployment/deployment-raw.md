...
In Flyte v2, **deployment** is the process of moving your code and task definitions from your local development environment to the Flyte backend, making them available for execution on the platform. Unlike traditional deployment where you deploy applications, in Flyte you deploy **Task Environments** - containers of related tasks that share common configuration, dependencies, and runtime settings.

### What Gets Deployed

When you deploy a Task Environment, several components are registered with the Flyte backend:

1. **Task Environment Metadata** - Configuration, resource requirements, dependencies
2. **Task Templates** - Serialized definitions of each `@env.task` function
3. **Code Bundle** - Your Python source code packaged and uploaded to cloud storage
4. **Container Images** - Built and pushed to your container registry (if custom images are specified)

**Important**: Your actual Python functions are not baked into container images. Instead, they're packaged separately and injected at runtime using Flyte's "fast registration" architecture.
...
Deploy the task environment using the CLI:
...
**Key points:**
- Specify the **file** (`my_example.py`) and the **variable name** (`env`)
- The variable name refers to the `TaskEnvironment` object, not its `name` property
- This deploys the environment and all tasks decorated with `@env.task`

### SDK Deployment

Deploy the same environment programmatically:
...
if __name__ == "__main__":
    ...
    print(f"Deployed: {deployment[0].summary_repr()}")
```

Both approaches achieve the same result - your task environment becomes available on the Flyte backend.
...
The `flyte.deploy()` function and the `flyte deploy` CLI command accept several options that control how the deployment is performed.

### Command Syntax

```bash
flyte deploy [OPTIONS] <PATH> [TASK_ENVIRONMENT_VARIABLE]
```
...
# Deploy all environments in directory
flyte deploy --all ./src

# Recursive deployment in directory tree
flyte deploy --recursive ./project

# Recursive deployment with specific copy style
...
| `--copy-style` | | choice | `loaded_modules` | Code bundling strategy |
...
| Option | Behavior | File Pattern |
|--------|----------|--------------|
| *none* | Deploy single specified environment | `<file>` → `env_var` |
| `--all` | Deploy all environments in path | `<file>` → all `TaskEnvironment` objects |
| `--recursive` | Deploy environments in directory tree | `<dir>/**/*.py` → all `TaskEnvironment` objects |
| `--all --recursive` | Deploy everything recursively | `<dir>/**/*.py` → all `TaskEnvironment` objects |

### Practical Examples

**Development workflow:**
```bash
# Quick single environment deployment
...
```bash
# Deploy all environments in current directory
flyte deploy --all .

# Deploy entire project tree with comprehensive bundling
...
# Deploy with custom image mappings
flyte deploy --image base=ghcr.io/org/base:v1.0 --image gpu=ghcr.io/org/gpu:v2.0 app.py env
```

**Monorepo deployment:**
```bash
# Deploy from specific root directory
flyte deploy --root-dir ./services/ml --recursive ./services/ml

# Deploy ignoring load errors in large codebase
flyte deploy --recursive --ignore-load-errors ./services
```

### Option Interactions

- **`--recursive`/`--all`** control **environment discovery scope**
- **`--copy-style`** controls **code bundling strategy**
- **Single code bundle** shared across all discovered environments
- **`--copy-style none`** requires **`--version`** parameter
- **`--image`** mappings apply to all discovered environments
...
This is a critical step that packages your code for deployment. The `copy_style` parameter controls what files get bundled:

#### Copy Style Options

**`"loaded_modules"` (Default - Smart Selection):**
```python
# Scans sys.modules for user modules loaded from your source directory
sys_modules = list(sys.modules.values())
files_to_bundle = []
for module in sys_modules:
    if (module.__file__ and
        is_in_source_directory(module.__file__) and
        not_in_site_packages(module.__file__) and
        not_in_system_prefix(module.__file__)):
        files_to_bundle.append(module.__file__)
```
- **Includes**: Only Python modules actually loaded and used from your project
- **Excludes**: System packages, site-packages, Flyte SDK, standard library
- **Best for**: Most projects (optimal bundle size and speed)

**`"all"` (Comprehensive Selection):**
```python
# Recursively includes all files in your project directory
files_to_bundle = []
for root, dirs, files in os.walk(source_directory):
    for file in files:
        if not should_ignore(file):  # Respects .gitignore-like rules
            files_to_bundle.append(os.path.join(root, file))
```
- **Includes**: All files in project directory (Python and non-Python)
- **Best for**: Projects with dynamic imports, data files, or complex dependencies

**`"none"` (No Bundling):**
- **Includes**: No code bundle is created
- **Requirements**: Must specify explicit `version` parameter
