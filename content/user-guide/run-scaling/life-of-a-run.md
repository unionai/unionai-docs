...
1. **Upload inputs**: The SDK uploads the run's inputs directly to the data-plane object store using a signed URL, the same way it uploaded the code bundle. The input values never pass through the control plane.
...
sequenceDiagram
    ...
    Client->>ObjStore: Upload inputs (signed URL)
