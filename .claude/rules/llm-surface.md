...
transform rule, so the browser URL does not change). Keep `content/api-reference/flyte-context.md` — the reader-facing description, and the only
page on this line covering the surface — consistent with any pipeline change. (The v2 line also
has `contributing-docs/llm-docs.md`; this line does not.)
