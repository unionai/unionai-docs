...
To create a demo cluster, run: :: ./bin/uctl demo start To remove a demo
cluster, run: :: ./bin/uctl demo teardown To check the status of the
demo container, run: :: ./bin/uctl demo status To execute commands
inside the demo container, use exec: :: ./bin/uctl demo exec \-- pwd
...
-   `uctl`{.interpreted-text role="doc"} - uctl is used to interact with
    Union Cloud
-   `uctl_demo_exec`{.interpreted-text role="doc"} - Executes
    non-interactive command inside the demo container
-   `uctl_demo_reload`{.interpreted-text role="doc"} - Power cycle the
    Flyte executable pod, effectively picking up an updated config.
-   `uctl_demo_start`{.interpreted-text role="doc"} - Starts the Flyte
    demo cluster.
-   `uctl_demo_status`{.interpreted-text role="doc"} - Gets the status
    of the demo environment.
-   `uctl_demo_teardown`{.interpreted-text role="doc"} - Cleans up the
    demo environment
