...
Generates a `uctl` config file in the user's home directory.

## Synopsis

Creates a `uctl` config file in `union` directory i.e `~/.union`.
...
Generate uctl config with a storage provider: :
