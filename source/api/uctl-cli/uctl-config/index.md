...
-   `uctl`{.interpreted-text role="doc"} - uctl is used to interact with
    Union Cloud
-   `uctl_config_discover`{.interpreted-text role="doc"} - Searches for
    a config in one of the default search paths.
-   `uctl_config_docs`{.interpreted-text role="doc"} - Generate
    configuration documentation in rst format
-   `uctl_config_init`{.interpreted-text role="doc"} - Generates a
    Flytectl config file in the user\'s home directory.
-   `uctl_config_validate`{.interpreted-text role="doc"} - Validates the
    loaded config.
