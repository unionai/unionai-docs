...
Tasks are the fundamental units of compute in {@= product_name =@}.
...
Under the hood, {@= product_name =@} uses the [Flyte type system](https://docs.flyte.org/en/latest/api/flyteidl/docs/core/core.html#flyteidl-core-types-proto) and translates between the Flyte types and the SDK language types, in this case Python.
...
While (most) tasks are locally executable, when a task is deployed to {@= product_name =@} as part of the registration process it is containerized and run in its own independent Kubernetes pod.
...
For example, a task that requires a GPU can be deployed to {@= product_name =@} with a GPU-enabled container image, while a task that requires a specific version of a software library can be deployed with that version of the library installed.
{@@ elif serverless @@}
...
One of the most powerful features of {@= product_name =@} is the ability to run widely differing computational workloads as tasks with a single workflow.

Because of the way that {@= product_name =@} is architected, tasks within a single workflow can differ along many dimensions. While the total number of ways that tasks can be configured is quite large, the options fall into three categories:
...
The ability to build workflows from such a wide variety of heterogeneous tasks makes {@= product_name =@} uniquely flexible.
...
The `@{@= union_flytekit_lower =@}.task` decorator can take a number of parameters that allow you to configure the task's behavior.
